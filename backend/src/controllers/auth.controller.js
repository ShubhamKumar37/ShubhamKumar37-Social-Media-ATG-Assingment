...
const sendOtp = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, 'OTP sent successfully', createOtp)
    );
});
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const userExist = await User.findOne({ userName }).select("-password -passwordResetToken -passwordTokenExpiry");
    ...
    return res.status(200).json(
        new ApiResponse(200, 'User logged in successfully', userExist)
    );
});
...
const signupUser = asyncHandler(async (req, res) => {
    ...
    const otpExist = await OTP.findOne({ email, otp }).sort({ createdAt: -1 }).limit(1);
